...
export const data = [
  ...
  { tema: '♊️', nombre: 'Gemini', link: '#gemini' },
  { tema: '✨', nombre: 'OpenIA', link: './openia' },
];
...
- **Graph-powered workflows**: Introducen la clase **Workflow**, que permite modelar procesos complejos como un grafo dirigido (Directed Graph). A diferencia de las orquestaciones lineales, un grafo permite una flexibilidad total para diseñar flujos de trabajo sofisticados:
    - **Nodos**: Representan unidades de trabajo individuales. Un nodo puede ser un Agente, una función de Python, una herramienta (tool) o incluso otro Workflow anidado.
    - **Aristas (Edges)**: Definen el camino que siguen los datos y el control. Las aristas conectan los nodos y pueden incluir lógica de enrutamiento (routers) para decidir dinámicamente el siguiente paso basado en el estado actual o el output de un nodo.
    - **Control Híbrido**: Es su mayor ventaja, ya que permite mezclar nodos deterministas (código tradicional para validaciones o cálculos) con nodos generativos (IA para razonamiento), asegurando que los procesos críticos sigan reglas de negocio estrictas.
    - **Topologías Flexibles**: Puedes expresar flujos secuenciales, ejecuciones en paralelo, bifurcaciones condicionales y ciclos de retroalimentación (loops) dentro de una única estructura coherente.
...
root_agent = Workflow(
    ...
    edges=[
        ...
        (classifier, agent_ventas, lambda res: res.category == "ventas"),
        (classifier, agent_soporte, lambda res: res.category == "soporte"),
    ]
)
```

`edges` define las conexiones y la lógica de transición, permitiendo que el flujo no sea solo una cadena lineal, sino un mapa inteligente de decisiones.

![alt text](image-8.png)

- **Task-based Collaboration**: Introduce un parámetro **mode** en los sub-agentes que proporciona un control granular sobre cómo un agente coordinador delega trabajo a especialistas. Esta técnica permite optimizar la interacción y el consumo de tokens según la naturaleza de la tarea delegada:
    - **Mode `chat`**: El sub-agente toma el control total de la conversación. Puede interactuar directamente con el usuario durante múltiples turnos para recolectar información o aclarar dudas antes de devolver el control al coordinador. Es ideal para procesos de "onboarding" o configuraciones guiadas.
    - **Mode `task`**: El sub-agente recibe una tarea específica y trabaja de forma autónoma (usando sus propias herramientas si es necesario) hasta completarla. Al finalizar, entrega un resultado estructurado al coordinador. El usuario no ve el proceso interno del sub-agente, solo el resultado final.
    - **Mode `single_turn`**: Es la forma más rápida y ligera de delegación. El coordinador envía una solicitud y el sub-agente responde en un único turno, sin mantener historial previo ni realizar loops internos complejos. Es perfecto para tareas de extracción, traducción o validaciones rápidas que pueden ejecutarse en paralelo.
...
Es una plataforma de GCP para construir, escalar, gobernar y optimizar agentes IA. 

- **Build**: Posee herramientas para la creacion de agentes. Agent Studio para development low-code, ADK para code-first engineering, sub-agent para sistemas multi-agente entre otros sistemas
- **Scale**: Infraestructura para ejecutar agentes en produccion. Agent Runtime para deployment, Agent Memory Bank para contextos a largo-plazo, Agent Sessions para trackear conversaciones y Agent Sandbox para ejecutar de manera segura operaciones riesgosas. 
- **Govern**: 

## `InMemorySessionService`

La clase InMemorySessionService almacena todos los datos de la sesión en la memoria. Esto significa que, si el servidor se reinicia o si se redirecciona al usuario a otro servidor, se perderán los datos de la sesión del usuario.

Cada solicitud que tu cliente envía al agente contiene un ID de sesión que el agente usa para asociar tu entrada con una conversación específica.
Sin embargo, cuando se detuvo y reinició el servidor, se perdieron todos los datos de la sesión que estaban en la memoria.
Por lo que ahora, tu cliente envía una solicitud con un ID de sesión que no está en la memoria, y el servidor rechazará el intento de continuar una conversación que no conoce.
...
Para agregar persistencia de la sesion en la app, se debe reemplazar a `InMemorySessionService` por `VertexAiSessionService`

Cuando es implementado, existe una latencia adicional en el uso del agente. Esto se debe a que los datos de la sesión se leen y escriben en un servicio fuera de la aplicación que entrega el agente, y cada llamada incurre en cierta latencia.

Cuando se probó la app con InMemorySessionService, el restablecimiento del servidor hizo que fallara.

Sin embargo, esta vez, como la sesión se almacenó en el servicio Agent Platform Sessions, el historial de conversaciones estaba disponible y se usó como contexto para formular la respuesta.
