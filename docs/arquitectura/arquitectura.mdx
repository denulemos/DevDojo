...
export const data = [
  { tema: '🧱', nombre: 'Microfrontends', link: 'patrones/microfrontends'},
  { tema: '🛠️', nombre: 'System Design', link: 'systemdesign/systemdesign' },
];
...
- Implementar en sistemas donde la escalabilidad es critica
- Cuando no es necesaria la consistencia inmediata de la informacion, ya que puede haber un retraso entre la escritura y la lectura de los datos.
...
Esto permite que el CQRS tenga una sincronizacion mas sencilla, ya que el estado del sistema se puede reconstruir a partir de la secuencia de eventos.

Es habitual ver al Event Sourcing junto con el CQRS, aunque no es necesario, se pueden usar ambos de forma independiente.

- Tratamiento independiente de las lecturas y las escrituras, ambas pueden ser optimizadas
- Mejor escalabilidad, ya que cada stack puede ser escalado de forma independiente
- Aniade mucha complejidad al sistema, ya que se deben manejar ambos stacks y la sincronizacion entre ambos.
- Se puede dar mucha redundancia entre ambos stacks, ya que ambos pueden almacenar la misma informacion de formas distintas.